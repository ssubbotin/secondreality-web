/**
 * Decoder for Future Crew's `.U` / `.UH` picture format (the converter output `LBM2U.EXE` writes).
 *
 * Layout (verified against `ALKU/FONA.UH` vs the incbin'd raw `ALKU/FONA.INC`, and `ALKU/HOI.U`):
 *
 ...
 *
 * The shipped FONA.UH / HOI.U store the pixel data uncompressed: after the palette come 5 bytes of
 * `add`-related metadata, then the `wid × hig` raw indices, then a single trailing pad byte (so the raw
 * pixel block ends exactly one byte before EOF, verified for both FONA.UH and HOI.U). The format's
 * general case is per-row RLE (a control byte with bit7 set introduces a run of the following byte,
 * bit7 clear a literal copy). This decoder handles both: if the file's tail matches an exact raw block it
 * copies it, otherwise it RLE-decodes from the end of the canonical 768-byte palette. Only the first 256
 * palette entries are returned (the on-disk file may carry extra overscan colours), matching where
 * `decodePicture` lands.
 */
...
export function decodeU(buf: ArrayBuffer | Uint8Array): DecodedU {
  ...
  // `pixelStart + total + 1 === fileSize` (verified for FONA.UH and HOI.U). When the geometry doesn't fit
  // that raw layout, fall through to the RLE decode from the canonical palette end.
