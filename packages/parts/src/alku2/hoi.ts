 ...
 * pixels live at paragraph `add` (`hzpic + add*16`, i.e. `16 + 768 = 784`), 640 bytes per row, panned
 * into the field by `do_scroll` / `outline()` (`ALKU/ASMYT.ASM`).
 *
 * This is a distinct read path from the FONA glyph-sheet `decodeU`: the horizon picture must use the
 * `hzpic+16` palette and `add*16` pixel offset exactly as `MAIN.C` does, or the sky colours and the
 * per-pixel scroll land wrong (the glyph-sheet `decodeU` heuristic places FONA one byte earlier, where
 * the incbin'd `FONA.INC` sits, and never uses its palette).
 */
