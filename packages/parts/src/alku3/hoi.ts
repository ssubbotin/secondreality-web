 ...
 *
 * This deliberately uses the `hzpic+16` palette and `add*16` pixel offset exactly as `MAIN.C` does
 * (distinct from the engine's `decodeU` glyph-sheet path, whose incbin'd `FONA.INC` sits one byte earlier
 * and whose palette is unused).
 */
