 ...
 * row=uv.y·ROTO_H), sampled from the tiling 256×256 RGB texture (the authentic LENS picture with its
 * palette baked in; RepeatWrapping handles the mod-256 wrap), scaled by the brightness fade. The CPU
 * sets the basis uniforms each frame (see affine.ts). The texture is RGB (not indexed) so the modern
 * LinearFilter blends real colours — interpolating palette indices would smear across the palette bands.
 */
